...
type EmployeesMasterAccordion = null | "roleLabels" | "department";
...
const Employees = () => {
  ...
  const [masterAccordion, setMasterAccordion] =
    useState<EmployeesMasterAccordion>(null);

  const toggleMasterAccordion = (panel: Exclude<EmployeesMasterAccordion, null>) => {
    setMasterAccordion((prev) => (prev === panel ? null : panel));
  };
  ...
  }, [masterAccordion, labels, rolesInOrder]);
  ...
  const hasUnsavedRoleLabelChanges = useMemo(
    () =>
      masterAccordion === "roleLabels" &&
      rolesInOrder.some(
        (r) => (roleLabelDraft[r] ?? "") !== (labels[r] ?? "")
      ),
    [masterAccordion, roleLabelDraft, labels, rolesInOrder]
  );
  ...
  const showRoleLabelResetToDefaultsButton = useMemo(
    ...
    [masterAccordion, roleLabelDraft, defaultLabels, rolesInOrder]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden divide-y divide-gray-100">
        <div>
          <button
            type="button"
            onClick={() => toggleMasterAccordion("roleLabels")}
            className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left text-sm font-medium text-gray-800 hover:bg-gray-50/80 transition-colors"
            aria-expanded={masterAccordion === "roleLabels"}
          >
            <span>役職の表示名を編集</span>
            <svg
              className={`h-4 w-4 text-gray-500 flex-shrink-0 transition-transform ${masterAccordion === "roleLabels" ? "rotate-180" : ""}`}
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden={true}
            >
          ...
          {masterAccordion === "roleLabels" && (
            <div className="px-4 pb-4 pt-0 sm:px-6 sm:pb-6 border-t border-gray-100 bg-white">
        ...
        <div>
          <button
            type="button"
            onClick={() => toggleMasterAccordion("department")}
            className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left text-sm font-medium text-gray-800 hover:bg-gray-50/80 transition-colors"
            aria-expanded={masterAccordion === "department"}
          >
            <span>部署マスタ編集（部・課）</span>
            <svg
              className={`h-4 w-4 text-gray-500 flex-shrink-0 transition-transform ${masterAccordion === "department" ? "rotate-180" : ""}`}
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden={true}
            >
          ...
          {masterAccordion === "department" && (
            <div className="border-t border-gray-100 p-3 sm:p-4 bg-gray-50/40">
